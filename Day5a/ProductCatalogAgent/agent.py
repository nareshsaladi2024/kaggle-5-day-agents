"""
Product Catalog Agent - External Vendor Agent
Exposed via A2A protocol for consumption by other agents.

This agent simulates an external vendor's product catalog service that provides
product information. In production, this would run on the vendor's infrastructure.
"""
...
try:
    ...
    setup_adk_logging(agent_name="product_catalog_agent", file_only=True)
except ImportError:
...
product_catalog_a2a_app = to_a2a(
    product_catalog_agent, port=PORT  # Port where this agent will be served
)

print(f"✅ Product Catalog Agent is now A2A-compatible!")
print(f"   Agent will be served at: http://localhost:{PORT}")
print(f"   Agent card will be at: http://localhost:{PORT}/.well-known/agent-card.json")
print("   Ready to start the server...")

# For uvicorn server, expose the app
app = product_catalog_a2a_app

# Define root_agent at module level for ADK web server compatibility
root_agent = product_catalog_agent

# Ensure logging is maintained after agent creation
try:
    ensure_debug_logging(agent_name="product_catalog_agent")
except NameError:
    pass
