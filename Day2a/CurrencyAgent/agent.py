...
Demonstrates custom function tools and agent tools.

This agent shows:
- Custom function tools (get_fee_for_payment_method, get_exchange_rate)
- Using another agent as a tool (CalculationAgent)
...
try:
    ...
    setup_adk_logging(agent_name="currency_agent", file_only=True)
except ImportError:
...
# Create a Calculation Agent that uses code execution
...
# Basic Currency Agent (with function tools only)
...
# Enhanced Currency Agent (with function tools + agent tool)
...
print("   Tools: get_fee_for_payment_method, get_exchange_rate, calculation_agent")
print("   Uses AgentTool to delegate calculations to specialist agent")
...
try:
    ensure_debug_logging(agent_name="currency_agent")
except NameError:
    pass
